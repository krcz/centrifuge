...
use crate::reflexive::{is_reflexive_cid, parse_ligation_bytes, reflexive_to_data_cid, resolve_ligation};
...
impl<'a, S: Store + ?Sized> StoreCursor<'a, S> {
    ...
    pub fn ipld(&self) -> Result<Ipld, ExportError<S::Error>> {
        ...
        serde_ipld_dagcbor::from_slice(&bytes)
            .map_err(|e| ExportError::Format(format!("value parse error for {}: {}", self.value_cid, e)))
    }
    ...
    pub fn follow_bond(
        ...
        let (schema_cid, schema_scope) =
            resolve_schema_cid(self.store, self.schemas, inner_schema_cid, &self.schema_scope)?;
        Ok(Self::with_state(
            self.store,
            self.schemas,
            value_cid,
...
fn load_schema_recursive_inner<S: Store + ?Sized>(
    ...
    match cell.value() {
        Structure::Sequence(inner) | Structure::Bond(inner) => {
            let _ = load_schema_recursive_inner(store, schemas, inner.cid(), &resolved_scope, visited)?;
        }
        ...
        Structure::Record(fields) | Structure::Tagged(fields) => {
            for bond in fields.values() {
                let _ = load_schema_recursive_inner(store, schemas, bond.cid(), &resolved_scope, visited)?;
            }
        }
        Structure::Map { key, value } | Structure::OrderedMap { key, value } => {
            let _ = load_schema_recursive_inner(store, schemas, key.cid(), &resolved_scope, visited)?;
            let _ =
    ...
    while is_reflexive_cid(&resolved_cid) {
        ...
        let (next_cid, next_scope) = resolve_ligation(ligation, &resolved_scope).ok_or_else(|| {
            match parse_ligation_bytes(resolved_cid.hash().digest()) {
                ...
                None => ExportError::Format(format!("invalid ligation payload for {}", resolved_cid)),
            }
        })?;
...
pub(crate) fn resolve_reflexive_edge<S: Store + ?Sized>(
    ...
    resolve_ligation(ligation, scope).ok_or_else(|| match parse_ligation_bytes(cid.hash().digest()) {
        Some(Ligation::Ligase(_)) => ExportError::EmptyLigase,
        Some(Ligation::Slot(index)) => ExportError::SlotOutOfRange(index),
